...
mod map;
pub use map::FallibleMap;

mod prune;
mod prune_depth;
mod prune_path;
pub use prune::{FallibleBinaryPrune, FalliblePrune};
pub use prune_depth::FalliblePruneDepth;
pub use prune_path::{FallibleBinaryPrunePath, FalliblePrunePath};

pub trait FallibleTreeIteratorBase<Value, Children, Err>:
    ...
    Self: Sized,
{
    ...
    /// ```
    #[must_use]
    fn prune_depth(self, depth_limit: usize) -> FalliblePruneDepth<Value, Children, Err, Self> {
        FalliblePruneDepth {
            inner: self,
            depth: depth_limit,
    ...
    /// ```
    #[must_use]
    fn map_tree<F, Output>(self, f: F) -> FallibleMap<Value, Children, Err, Self, F, Output>
    where
        F: FnMut(Value) -> Output,
    {
        FallibleMap::new(self, f)
    }
}

pub trait FallibleTreeIterator<Value, Children, Err>:
    FallibleTreeIteratorBase<Value, Children, Err>
where
    Self: Sized,
{
    ...
    /// ```
    #[must_use]
    fn prune<F>(self, f: F) -> FalliblePrune<Value, Children, Err, Self, F>
    where
        F: FnMut(&Value) -> bool,
    {
        FalliblePrune::new(self, f)
    }
    ...
    #[must_use]
    fn prune_path<F>(self, f: F) -> FalliblePrunePath<Value, Children, Err, Self, F>
    where
        F: FnMut(&[usize], &Value) -> bool,
    {
        FalliblePrunePath::new(self, f)
    }
    ...
    ///     result);
    /// ```
    fn collect_tree(mut self) -> Result<Option<Tree<Value>>, Err> {
    ...
    /// ```
    fn fold_tree<F, Output>(mut self, mut f: F) -> Result<Option<Output>, Err>
    where
        F: FnMut(Vec<Output>, Value) -> Output,
    {
        ...
        Ok(if let Some(root) = inversion_stack.pop() {
...
pub trait FallibleBinaryTreeIterator<Value, Children, Err>:
    FallibleTreeIteratorBase<Value, Children, Err>
where
    Self: Sized,
{
    ...
    /// ```
    #[must_use]
    fn prune<F>(self, f: F) -> FallibleBinaryPrune<Value, Children, Err, Self, F>
    where
        F: FnMut(&Value) -> bool,
    {
        FallibleBinaryPrune::new(self, f)
    }
    ...
    /// [`current_path`](TreeIteratorBase::current_path) for more details).
    #[must_use]
    fn prune_path<F>(self, f: F) -> FallibleBinaryPrunePath<Value, Children, Err, Self, F>
    where
        F: FnMut(&[usize], &Value) -> bool,
    {
        FallibleBinaryPrunePath::new(self, f)
    }
    ...
    ///     result);
    /// ```
    fn collect_tree(mut self) -> Result<Option<BinaryTree<Value>>, Err> {
        ...
        Ok(keeping_stack.pop().map(|tuple| tuple.1))
    }
