...
pub trait FallibleTreeCollectionIteratorBase<Value, Children, Err>: Iterator<Item = Result<Value, Err>>
where
    Self: Sized,
{
    ...
    ///     result);
    /// ```
    fn prune_depth(self, depth_limit: usize) -> FallibleCollectionPruneDepth<Value, Children, Err, Self> {
    ...
    Self: Sized,
{
    ...
    ///     result);
    /// ```
    fn collect_trees(mut self) -> Result<Vec<Tree<Value>>, Err> {
        let mut result: Vec<Tree<Value>> = Vec::new();
        let mut keeping_stack: Vec<Tree<Value>> = Vec::new();
        while let Some(item) = self.next() {
            let item = item?;
            let mut current_depth = self.current_path().len();
            if current_depth != 0 {
                current_depth -= 1;
            }
    ...
    Self: Sized,
{
    ...
    ///     result);
    /// ```
    fn collect_trees(mut self) -> Result<Vec<BinaryTree<Value>>, Err> {
